describe('Fluxo de compra com cartão', () => {
  ...
  it('Adicionar dois produtos ao carrinho e finalizar pagamento', () => {
    ...
    cy.get('#inputCupom').type('NANA1')
    cy.get('#btnAplicarCupom').click()
    cy.contains('Desconto de Boas-Vindas — R$ 10,00 de desconto').should('be.visible')
    ...
    cy.get('#sumDesconto').should('have.text', '- R$ 10,00')
    cy.get('#sumTotal').should('have.text', 'R$ 216,40')
...
cy.url().should('match', /\/historico\/\d+/)
    ...
    cy.contains('R$ 216,40').should('be.visible')//aquiR$ 220,90
  })
}) 